...
trips.interp.nomigr<-clean_trips_interp %>% 
    ...
    mutate(day=match(format(DateEnd,format="%F"), unique(date)))
...
ggplot(trips.interp.nomigr,aes(x=day,y=Distmaxkm,
    ...
    title("Maximal distance to colony over time by individual")

glmm.dist<-lmer(log(Distmaxkm) ~ day + (1 | id),data=trips.interp.nomigr)
     
plot(glmm.dist)
summary(glmm.dist,corr=F)
...
ggplot(trips.interp.nomigr,aes(x=day,y=TripDurh,
    ...
    geom_point()+
    geom_smooth(method="lm",alpha=0.2)+
   title("Trip duration over time by individual")

glmm.dur<-lmer(log(TripDurh) ~ day + (1 | id),data=trips.interp.nomigr)
     
plot(glmm.dur)
summary(glmm.dur,corr=F)
...
ggplot(trips.interp.nomigr,aes(x=day,y=TotalPathkm,
    ...
    geom_point()+
    geom_smooth(method="lm",alpha=0.2) +
    title("Total trip distance over time by individual")


glmm.dur<-lmer(log(TotalPathkm) ~ day + (1 | id),data=trips.interp.nomigr)
