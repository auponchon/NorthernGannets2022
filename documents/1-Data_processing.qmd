...
As data has irregular time intervals between consecutive locations, we linearly interpolated data to have one location every 15min., as 96% of data have a time interval of less than 15min. We then compared each raw clean trips with interpolations. It appears that for 1 individual (222276), interpolation badly recreates 2 trips, as they cross Brittany following the coastline. Note that maps are bounded at \> 47° N to have a better focus on the study area before migration.
...
glmm.dur<-lmerTest::lmer(log(LandDurh) ~ day + (1 | id), data=lands)
