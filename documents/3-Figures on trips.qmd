---
title: "3-Trips characteristics with LMM"
format: html
editor: visual
---
...
kbl(sum.ids,caption="Sample size") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))

sum.id<- trips %>% 
    group_by(site,id) %>% 
    summarize(across(c(Distmaxkm, TripDurh, TotalPathkm), ~ mean(.x, na.rm = TRUE))) %>% 
    as.data.frame()
...
kbl(sum.ids,caption="Summary of trip characteristics") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))

kbl(sum.lands,caption="Summary of next attendance") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
...
anova(glmm.dist.rouz)
summary(glmm.dist.bass)
anova(glmm.dist.bass)
...
rr<-simulateResiduals(glmm.dist.rouz,plot=T)
ss<-simulateResiduals(glmm.dist.bass,plot=T)
...
bb<-simulateResiduals(glmm.dur.rouz,plot=T)
cc<-simulateResiduals(glmm.dur.bass,plot=T)
...
anova(glmm.path.rouz)
summary(glmm.path.bass)
anova(glmm.path.bass)
...
ff<-simulateResiduals(glmm.path.rouz,plot=T)
gg<-simulateResiduals(glmm.path.bass,plot=T)
...
anova(glmm.land.rouz)
summary(glmm.land.bass)
anova(glmm.land.bass)
...
dd<-simulateResiduals(glmm.land.rouz,plot=T)
qq<-simulateResiduals(glmm.land.bass,plot=T)
