...
land.rouzic<-rawLoc.rouzic %>% 
 ...
 trips<-trips.bassrock %>% 
       ...
       mutate(day=match(format(DateEnd,format="%F"), 
                     sort(unique(format(DateEnd,format="%F")))))
...
glmm.dist.lme.rouzic<-lme(sqrt(Distmaxkm) ~ day, random=~1 | id , data=trips.rouzic)
glmm.dist.lme.bassrock<-lme(sqrt(Distmaxkm) ~ day, random=~1 | id , data=trips.bassrock)


#predict fitted values and CI
newdatrouz$pred<-predict(glmm.dist.lme.rouzic, newdata=newdatrouz,level=0)
...
geom_point() +
 geom_line(data=newdatall,aes(x=day,y=pred^2),lwd=2) +
...
glmm.dur.lme.rouzic<-lme(sqrt(TripDurh) ~ day, random=~1 | id , data=trips.rouzic)
glmm.dur.lme.bassrock<-lme(sqrt(TripDurh) ~ day, random=~1 | id , data=trips.bassrock)
...
#plot with separate datasets trip duration
glmm.path.lme.rouzic<-lme(sqrt(TotalPathkm) ~ day, random=~1 | id , data=trips.rouzic)
glmm.path.lme.bassrock<-lme(sqrt(TotalPathkm) ~ day, random=~1 | id , data=trips.bassrock)
...
glmm.land.lme.rouzic<-lme(sqrt(LandDurh) ~ day, random=~1 | id , data=land.rouzic)
glmm.land.lme.bassrock<-lme(sqrt(LandDurh) ~ day, random=~1 | id , data=land.bassrock)
...
tiff(here("outputs","Figure_LMM.tif"),width=7000,height=4500,res=600)
